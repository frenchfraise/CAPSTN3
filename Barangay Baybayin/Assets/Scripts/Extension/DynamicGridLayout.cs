...
public class DynamicGridLayout : LayoutGroup
{
    ...
    protected virtual (int rows, int columns) calculateRowsAndColumns()
    {
        ...
        switch (gridPriority)
        {
            case DynamicGridLayout.GridPriority.Rows:
                return (Mathf.CeilToInt(sqrRT), Mathf.RoundToInt(sqrRT));
            case DynamicGridLayout.GridPriority.Columns:
                return (Mathf.RoundToInt(sqrRT), Mathf.CeilToInt(sqrRT));
            case DynamicGridLayout.GridPriority.FixedColumns:
                return (Mathf.CeilToInt((float)rectChildren.Count / columns), columns);
            case DynamicGridLayout.GridPriority.FixedRows:
    ...
    {
        return gridPriority == DynamicGridLayout.GridPriority.FixedColumns || gridPriority == DynamicGridLayout.GridPriority.Rows ? cellIndex % columns : cellIndex / rows;
    }
    ...
    public class DynamicGridLayoutEditor : Editor
    {
        ...
        {
            base.OnInspectorGUI();
            DynamicGridLayout dynamicGridLayout = (DynamicGridLayout)target;
            dynamicGridLayout.allowOutOfBounds = EditorGUILayout.Toggle(new GUIContent("Allow out of bounds", "Ignores the bounds of the screen for child alignment, In other words if not allowing out of bounds it will behave like unity layout, Means that if the cells are out of bounds it will snap to Upper or Left child alignment"), dynamicGridLayout.allowOutOfBounds);
            dynamicGridLayout.gridPriority = (GridPriority)EditorGUILayout.EnumPopup("Grid Priority", dynamicGridLayout.gridPriority);
            dynamicGridLayout.rows = EditorGUILayout.IntField("Rows", dynamicGridLayout.rows);
            dynamicGridLayout.columns = EditorGUILayout.IntField("Columns", dynamicGridLayout.columns);
            dynamicGridLayout.customCellSize = EditorGUILayout.Toggle(new GUIContent("Custom Cell Size", "When using custom cell size,Use layout element to customize the cells"), dynamicGridLayout.customCellSize);

            if (dynamicGridLayout.customCellSize)
            {
                EditorGUILayout.BeginHorizontal();
                EditorGUILayout.LabelField("Horizontal Size Preference");
                dynamicGridLayout.horizontalSizePreference = (ContentSizeFitter.FitMode)EditorGUILayout.EnumPopup(dynamicGridLayout.horizontalSizePreference, GUILayout.ExpandWidth(true));
                EditorGUILayout.EndHorizontal();
                EditorGUILayout.BeginHorizontal();
                EditorGUILayout.LabelField("Vertical Size Preference");
                dynamicGridLayout.verticalSizePreference = (ContentSizeFitter.FitMode)EditorGUILayout.EnumPopup(dynamicGridLayout.verticalSizePreference, GUILayout.ExpandWidth(true));
                EditorGUILayout.EndHorizontal();
                if (dynamicGridLayout.verticalSizePreference == ContentSizeFitter.FitMode.Unconstrained || dynamicGridLayout.horizontalSizePreference == ContentSizeFitter.FitMode.Unconstrained)
                {
                ...
                dynamicGridLayout.rootSpacingFromHighestCellInRow = EditorGUILayout.Toggle(dynamicGridLayout.rootSpacingFromHighestCellInRow, GUILayout.Width(14));
                EditorGUILayout.EndHorizontal();
                EditorGUILayout.BeginHorizontal();
                ...
                dynamicGridLayout.rootSpacingFromWidestCellInColumn = EditorGUILayout.Toggle(dynamicGridLayout.rootSpacingFromWidestCellInColumn, GUILayout.Width(14));
                EditorGUILayout.EndHorizontal();
            }
            else
            {
                ...
                GUI.enabled = !dynamicGridLayout.fitX;
                EditorGUILayout.LabelField("X", GUILayout.Width(8));
                dynamicGridLayout.cellSize.x = EditorGUILayout.FloatField(dynamicGridLayout.cellSize.x, GUILayout.MinWidth(55), GUILayout.Width(55), GUILayout.ExpandWidth(true));
                GUI.enabled = !dynamicGridLayout.fitY;
                EditorGUILayout.LabelField("Y", GUILayout.Width(8));
                dynamicGridLayout.cellSize.y = EditorGUILayout.FloatField(dynamicGridLayout.cellSize.y, GUILayout.MinWidth(50), GUILayout.Width(50), GUILayout.ExpandWidth(true));
                EditorGUILayout.LabelField("", GUILayout.ExpandWidth(true), GUILayout.MinWidth(60), GUILayout.Width(60));
                EditorGUILayout.EndHorizontal();
                GUI.enabled = true;
            }

            dynamicGridLayout.spacing = EditorGUILayout.Vector2Field("Spacing", dynamicGridLayout.spacing);
            if (!dynamicGridLayout.customCellSize)
            {
                dynamicGridLayout.magnetizeCells = EditorGUILayout.Toggle(new GUIContent("Magnetize Cells", "The Row/Column(depends on the grid priority) that has less cells will snap to child alignment grid"), dynamicGridLayout.magnetizeCells);
                dynamicGridLayout.fitX = EditorGUILayout.Toggle("Fit X", dynamicGridLayout.fitX);
                dynamicGridLayout.fitY = EditorGUILayout.Toggle("Fit Y", dynamicGridLayout.fitY);
            }

            dynamicGridLayout.CalculateLayoutInputHorizontal();
        }
    }
#endif
}
